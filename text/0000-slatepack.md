...
Plain text metadata is included with Slatepack messages to indicate how to handle the encryption if any for the slata data in addition to tracking versions for compatibility. It can be expanded in future versions with new fields that are safe to include as plain text. These fields are neither encrypted nor authenticated. 
...
```
{
  ...
  "payload": <age encrypted binary: binary seriaized slate>,
}
```
...
```
{
  "slatepack": [0, 2],
  "mode": 1,
  "payload": <age encrypted binary: sender slatepack address + binary seriaized slate>,
}
```
