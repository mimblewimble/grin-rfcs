...
- Title: Combine MMR Roots
- Authors: [David Burkett](mailto:davidburkett38@gmail.com)
- Start date: July 22nd, 2009 [edit and replace the day work begun, i.e. `Jan 15, 2019`]
...
Replace the kernel, output, and rangeproof MMR roots in the block header with a single commitment to all three.

# Motivation
[motivation]: #motivation

Block headers for Grin are 400 bytes, 5x larger than Bitcoin's 80 byte headers. These relatively large headers are required in order to sync a new node.
To make matters worse, the Grin protocol produces blocks 10x as often as bitcoin, due to its 1 minute block target. 6 months after Grin's launch, the headers of the 268000 blocks in the chain are over 100MB total, already over twice the size of all of bitcoin's headers since genesis.
Because of the size of these headers, storing them in memory the way most bitcoin clients do is not feasible long-term. This requires reading them from disk, results in slower lookup times (profiling I've done indicates header lookups are the biggest cause of Grin performance woes).
By reducing the 3 MMR roots to a single commitment, we get a 16% reduction in header size, and when download speeds are the bottleneck, this translates to a similar reduction in header sync time.
It also takes us one step closer to the possibility of keeping headers in memory, offering potentially significant performance improvements.
