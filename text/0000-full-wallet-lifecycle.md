...
Sensitive data, particularly new passwords and/or seed phrases that may be transmitted during API calls are also encrypted with **s**+**k** when used as either input parameters or return values.

![api_call](../assets/0000/0000-api-call.svg)

### InsecureAPI Mode

Calling the API in insecure mode is a simpler case of the above, where passwords, seed phrases and nonces are all sent in the clear. For consistency with the above exchange, an API Token is still returned during the initial call to the `open_wallet` function, and this token is used to store the XORed seed value internally. However this mode provides no protection against snooping attackers, and should **never** be used in a cross-machine configuration.

![unsafe-mode](../assets/0000/0000-unsafe-mode.svg)
